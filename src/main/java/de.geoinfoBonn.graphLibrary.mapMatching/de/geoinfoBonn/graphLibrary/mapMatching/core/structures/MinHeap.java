...
public class MinHeap<T> {
	...
	public static class HeapItem<T> {
		...
		public String toString() {
			return "HeapItem [key=" + key + ", value=" + value + ", index=" + index + "]";
		}
	}
	...
	public void increaseKey(HeapItem<T> item, double key) {
		item.setKey(key);
		heapify(item);
	}

	public void changeKey(HeapItem<T> item, double key) {
		if (item.getKey() > key) {
			this.decreaseKey(item, key);
		} else if (item.getKey() < key) {
			this.increaseKey(item, key);
		} else {
//			System.out.println("Key not changed, was the same before. (structures.MinHeap.changeKey)");
		}
	}
