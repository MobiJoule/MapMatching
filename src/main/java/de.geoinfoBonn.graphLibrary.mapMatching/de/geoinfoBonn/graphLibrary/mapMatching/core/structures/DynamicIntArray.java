 ...
 * used to transform template "int" to wanted datatype.
 * 
 * @author Axel Forsch
 * @version 1.0
 * @since 17. April 2018
...
public class DynamicIntArray {
	 ...
	 */
	public DynamicIntArray(int[] content) {
	...
	public void add(int element) {
		...
		if (idx == this.capacity()) {
			int[] doubledContent = new int[this.capacity() * 2];
	...
	public void remove(int index) {
		...
		if (this.size() < this.capacity() / 2) {
			int[] halvedContent = new int[this.capacity() / 2];
	...
	public void setCapacity(int capacity) {
		if (capacity < this.size()) {
			...
			// (in tests.DynamicIntArray.setCapacity(int))");
		}
